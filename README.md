...
- `GET /api/v1/public/{projectKey}/versions/by-version/{version}`：同时支持语义化版本号与可比较版本号查询指定版本。
- `POST /api/v1/public/{projectKey}/versions/check-update`：提交当前版本并自动返回是否更新、是否强制、阶段目标版本（里程碑优先）。
...
- 检查更新优先级：`current_version` 与 `current_comparable_version` 同时提交时，优先按可比较版本号判定。
