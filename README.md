...
- 请求统计：按项目统计各公开接口的调用情况，并提供统计大屏可视化
...
该开发栈为热重载模式：`packages/backend/src`、`web/`、`packages/ui/src` 以只挂载源码目录的方式绑定进容器（不挂载 `node_modules`，避免宿主机平台相关依赖与 pnpm 绝对路径软链接污染容器），改动源码后后端 `nest --watch`、前端 `next dev` 会自动重新编译，无需重新构建镜像。

由于 Windows / macOS 的绑定挂载不会向容器内投递 inotify 事件，开发栈已开启文件轮询（后端 `TSC_WATCHFILE`/`TSC_WATCHDIRECTORY`，前端 `WATCHPACK_POLLING`）。前端因此在容器内使用 webpack 而非 Turbopack —— Turbopack 的监听器无法感知绑定挂载的写入，且没有轮询开关。

开发栈还会把 PostgreSQL 端口映射到宿主机（默认 5432，可用 `VERHUB_POSTGRES_PORT` 覆盖），便于直接使用 psql 或 Prisma Studio 调试。
