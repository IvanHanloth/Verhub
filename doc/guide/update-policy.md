...
2. 判断当前版本是否在项目“可选更新范围”内
3. 判断当前版本是否为废弃版本
4. 判断是否触发里程碑拦截（必须先升到当前里程碑最新）
...
- `milestone`：里程碑标识（如 M1、M2），填写后该版本将作为里程碑版本。低于此版本的所有版本在更新检查时只能先更新到该里程碑版本，再继续后续更新。
- `is_deprecated`：勾选后该版本客户端必须更新至最近里程碑版本或更高版本

建议：

- `version` 与 `comparable_version` 分离管理，避免历史命名风格影响策略判断
- milestone 尽量使用稳定命名（如 M1/M2 或 YYYY-Qn）
