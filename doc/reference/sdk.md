...
- **本地时间声明**：每个请求（含 admin 与事件上报）默认带 `x-verhub-client-time`，
  值为设备本地时间加 UTC 偏移（如 `2026-09-24T10:00:00.123+08:00`），服务端据此
  按用户当地时间统计、校正设备时钟偏差。可在构造时关闭，见[本地时间声明](#本地时间声明)。
- **三类错误**：缺凭据等**本地前置校验失败**（请求没发出去）抛 `VerhubAuthError`
  ...
  `VerhubError`。
- **默认重试**：GET / HEAD 在连接失败与 502/503/504 时默认自动重试 3 次并指数退避；
...
- **重试**：GET / HEAD 在连接失败与 502/503/504 时默认自动重试 3 次并指数退避，
...
每个请求（含 admin 接口与事件上报，重试的每一次也算）默认带上
`x-verhub-client-time`：设备的本地墙钟时间，毫秒精度，外加显式的数字 UTC 偏移，
如 `2026-09-24T10:00:00.123+08:00`。偏移恒写 `±HH:MM`，UTC 也写 `+00:00` 而不是 `Z`。

服务端从这一个值里取两样东西：

- **偏移**：行为统计按用户当地时间折叠（如「一天中哪个钟点最活跃」）；
- **时刻**：与收到请求的时刻相比，得出设备时钟的偏差，用来校正离线补发事件的发生时间。

所以这个值在**每次发送前现取**，不是构造客户端时算一次。取不到合规值（运行环境拿不到
时区、历史日期带秒级偏移等）时不发这个头，请求照常进行；服务端按未上报处理。
浏览器页面卸载时走的 `sendBeacon` 设不了请求头，那一批事件同样按未上报处理。

| 语言            | 关闭方式                                                      |
| --------------- | ------------------------------------------------------------- |
| Python          | `VerhubClient(..., send_client_time=False)`（异步版同名参数） |
| TypeScript / JS | `new VerhubClient({ ..., sendClientTime: false })`            |
| Rust            | `VerhubClient::builder(...).without_client_time()`            |

实现上 Python 用标准库 `datetime`，TS / JS 只用 `Date`（不依赖 `Intl`），Rust 用
`chrono` 取本地偏移（std 没有这个接口）。
