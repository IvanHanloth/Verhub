...
export const PRIVACY_POLICY_UPDATED_AT = 1790208000 // 2026-09-24T00:00:00Z
...
无论应用调用哪一项能力，下列信息都会随请求发送：项目标识、平台类型（windows、linux、macos、ios、android、web、others 七者之一）、系统版本明细（例如「11」「ubuntu 24.04」）、SDK 名称与版本号（通过 User-Agent 发送）、发出请求时设备的本地时间及其 UTC 偏移（例如「+08:00」，不含时区名称或者地理位置）。

系统版本明细的读取范围严格限于操作系统版本本身，读取结果在发送前经过字符清洗并截断至 32 个字符。平台类型、系统版本明细与本地时间均可由应用在初始化时关闭。

处理目的：确定数据归属的项目；按平台筛选应向您展示的公告；统计客户端在各平台与各系统版本上的分布；在特定平台出现异常时缩小排查范围；按您所在时区统计活跃时段，并校正设备时钟偏差，使离线补发的行为事件落在正确的时间上。本地时间本身不单独保存，只保留其 UTC 偏移作为聚合统计的维度。
...
| 统计项 | 统计维度 | 保存内容 |
| --- | --- | --- |
| 接口调用量 | 项目、接口、自然小时、平台类型、国家与省市行政区划、UTC 偏移 | 计数 |
| 客户端版本分布 | 项目、版本号、自然小时、平台类型 | 计数 |
| 系统版本分布 | 项目、自然小时、平台类型、系统版本明细 | 计数 |
| 行为事件量 | 项目、事件名、自然小时、平台类型、国家与省市行政区划、UTC 偏移 | 计数 |
