...
export function DashboardShell({ children }: Props) {
  ...
  const themeIcons = { system: Laptop, light: Sun, dark: Moon }
  const themeLabels = { system: "跟随系统", light: "浅色模式", dark: "深色模式" }
  const ThemeIcon = themeIcons[currentTheme] ?? Laptop
  const themeToggleLabel = `主题：${themeLabels[currentTheme] ?? "跟随系统"}`

  function cycleTheme() {
    const index = themeOrder.indexOf(currentTheme)
    // The modulo always lands in range; the fallback only satisfies
    // noUncheckedIndexedAccess, which cannot see that.
    setTheme(themeOrder[(index + 1) % themeOrder.length] ?? "system")
  }
